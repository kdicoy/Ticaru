...
class HeaderClass extends React.Component {
  ...
  mobileMenuReducer = (result, link) => {
    ...
    const TheLink = {
      label: <div style={{ width: "100vw", padding: "5px" }}>{link.text}</div>,
      onClick: () => this.navigateRoute(link.to)
    };
          ...
          <Box direction="row" gap="xsmall">
            {links.map(link => {
              const TheLink = (
                <Anchor
                  key={link.to}
                  color={isCurrent(link.to, current) ? "neutral-2" : "accent-1"}
                  onClick={() => this.navigateRoute(link.to)}
                >
                  {link.text}
                </Anchor>
    ...
    return (
      ...
      >
        ...
        <Box justify="center" align="center" fill="horizontal">
          <Heading align="center" color="brand">
            Ticaru
          </Heading>
        </Box>
      </Box>
  ...
  render() {
    ...
    const headerBasedOnDisplay =
      size === "xsmall" || size === "small"
        ? this.renderHalfScreen()
